...
import { getFirestore, doc, getDoc, updateDoc, arrayUnion, increment }
...
// ── Track this view (fire-and-forget, non-blocking)
; (async () => {
  try {
    const propRef = doc(db, "properties", propertyId);
    await updateDoc(propRef, {
      viewCount: increment(1),
      viewLog: arrayUnion(Date.now()),
    });
  } catch (e) {
    // silently ignore — view tracking should never break the page
    console.warn("view track:", e);
  }
})();
...
function rerenderProperty() {
  ...
  if (p.ceilingHeight != null && String(p.ceilingHeight).trim() !== "")
    addRow("property.ceiling", `${p.ceilingHeight} cm`);
