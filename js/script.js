...
(() => {
  ...
  document.addEventListener("DOMContentLoaded", async () => {
        ...
        >
          ...
          <div class="card-body">
            ...
            <div class="d-flex gap-2">
              <a href="pages/property.html?id=${encodeURIComponent(p.id)}" class="btn btn-sm btn-outline-danger">Vezi Detalii</a>
            </div>
    ...
    try {
      ...
      if (!items.length) {
        if (agentId) {
          grid.innerHTML = `<p>Acest agent nu are imobile încă.</p>`;
        } else {
          grid.innerHTML = `<p>Nu există imobile încă.</p>`;
        }
